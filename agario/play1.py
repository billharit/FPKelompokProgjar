...
pygame.display.set_caption('PROGJAR AGARIO')
...
class Player:
    ...
    def renderText(self):
        ...
        screen.blit(font.render(text, True, (255, 255, 255)),
                    (int(self.x-self.r/2), int(self.y-(self.r/4))))
...
class Game():
    ...
    def __init__(self):
        ...
        self.anotherplayer = [Player((random.randrange(0, 255), random.randrange(0, 255), random.randrange(0, 255))), Player((random.randrange(0, 255), random.randrange(0, 255), random.randrange(0, 255))), Player((random.randrange(
            0, 255), random.randrange(0, 255), random.randrange(0, 255))), Player((random.randrange(0, 255), random.randrange(0, 255), random.randrange(0, 255))), Player((random.randrange(0, 255), random.randrange(0, 255), random.randrange(0, 255)))]
    ...
    def redraw(self):
        ...
        for i in range(len(self.randomlist)):
            color = (self.randomlist[i][0],
                     self.randomlist[i][1], self.randomlist[i][2])
            pygame.draw.circle(
                screen, color, (self.randomlist[i][3], self.randomlist[i][4]), 3)
    ...
    def updatedata(self, fullMsg):
        ...
        for i in range(len(fullMsg[1])):
            self.anotherplayer[i].setposition(
                fullMsg[1][i][0], fullMsg[1][i][1], fullMsg[1][i][2])
